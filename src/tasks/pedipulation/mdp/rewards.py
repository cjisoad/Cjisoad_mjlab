...
def tracking_lin_vel(env):
  ...
  vel = term.anchor_frame.to_frame(env.scene['robot'].data.root_link_lin_vel_w)
  cmd = term.command
...
def tracking_ang_vel(env):
  term = env.command_manager.get_term('stand')
  velocity = term.anchor_frame.to_frame(env.scene['robot'].data.root_link_ang_vel_w)
...
def ang_vel_xy(env):
  velocity = env.command_manager.get_term('stand').anchor_frame.to_frame(env.scene['robot'].data.root_link_ang_vel_w)
...
def feet_height_symmetry(env, asset_cfg=FRONT):
  heights = env.scene[asset_cfg.name].data.site_pos_w[:, asset_cfg.site_ids, 2]
  enabled = ~env.command_manager.get_term('stand').has_foot_target | ~_gate(env)
  return (heights[:, 0] - heights[:, 1]).abs() * enabled
