...
from src.tasks.stand.constants import JOINT_NAMES, DESIRED_ANGLES


@dataclass(kw_only=True)
class StandPositionActionCfg(ActionTermCfg):
  scale: float = .25
  delay: bool = True

  def build(self, env):
    return StandPositionAction(self, env)


class StandPositionAction(ActionTerm):
