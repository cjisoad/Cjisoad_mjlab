"""Source reward arithmetic expressed as individual mjlab reward terms."""
...
def tracking_lin_vel(env):
  vel = env.scene['robot'].data.root_link_lin_vel_b
  cmd = env.command_manager.get_command('stand')
  error = (cmd[:, 0] + vel[:, 2]).square() + (cmd[:, 1] - vel[:, 1]).square()
  return torch.exp(-error / .25) * _gate(env)


def tracking_ang_vel(env):
  error = env.command_manager.get_command('stand')[:, 2] - env.scene['robot'].data.root_link_ang_vel_b[:, 0]
  return torch.exp(-error.square() / .25) * _gate(env)


def lin_vel_z(env):
  return torch.exp(-env.scene['robot'].data.root_link_lin_vel_b[:, 0].abs() * 10)


def ang_vel_xy(env):
  return torch.exp(-env.scene['robot'].data.root_link_ang_vel_b[:, 1:3].norm(dim=-1))
