...
class StandGaussianDistribution(GaussianDistribution):
...
class StandPPO(PPO):
  ...
  def __init__(self, *args: Any, sym_coef: float = 1.0, **kwargs: Any) -> None:
    if kwargs.get("symmetry_cfg") is not None:
      raise ValueError("StandPPO supplies its own symmetry loss; symmetry_cfg must be None")
    super().__init__(*args, **kwargs)
    if self.actor.is_recurrent or self.critic.is_recurrent:
      raise ValueError("StandPPO requires the source feed-forward policy")
    if self.actor.obs_dim != 45 or tuple(self.actor.obs_groups) != ("actor",):
      raise ValueError("StandPPO requires one 45-dimensional actor observation group")
    if self.critic.obs_dim != 86:
      raise ValueError("StandPPO requires 86 critic observations")
    ...
    if self.actor.distribution.output_dim != 12:
      raise ValueError("StandPPO requires 12 canonical joint actions")
